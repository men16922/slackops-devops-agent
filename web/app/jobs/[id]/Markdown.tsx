...
// Claude 진단 결과가 내뱉는 구조만 다룬다: heading(#)·bold(**)·inline code(`)·
// 불릿/번호 리스트·blockquote(>)·코드펜스(```)·문단. 텍스트는 React 노드로 삽입되어
// 이스케이프되므로 XSS 안전(dangerouslySetInnerHTML 미사용).

// bold(**) → italic(*) → inline code(`) 순서로 매칭(긴 토큰 우선). 비탐욕 →
// `**a *b* c**` 같은 중첩 허용. bold/italic 안은 재귀 파싱(code 안은 리터럴).
const INLINE = /(\*\*([\s\S]+?)\*\*|\*([^*\n]+?)\*|`([^`]+?)`)/;
