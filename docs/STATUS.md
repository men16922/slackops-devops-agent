...
- 3-layer gate: `make check` → **492 passed** · `ruff` · `mypy src`(strict) · documentation-budget gate all green;
...
- **P1 central system-boundary audit (real EC2 rehearsal, 2026-07-15):** deployment provisions the 30-day
  ...
  write it (explicit `PutLogEvents` deny). Fresh EC2 logged `credential_refresh` + URL-free `proxy_denied`; audit env root `600`,
  state root `700`; artifact/instance removed.
- **P2 deterministic scope boundary (real EC2 rehearsal, 2026-07-15):** root-owned env fixes account/region/log-prefix/workspace;
  every command gets a fixed scope before adapter/executor and is rechecked before Claude. Out-of-prefix fetch denied before access,
  Worker emitted `policy_denied` with reason/scope, 24-hour window verified; artifact/instance removed.
...
- **D19 execution boundary + write credentials (local/CI, 2026-07-16):** measured that `--allowedTools 'Bash(echo:*)'` still ran
  `echo hi; whoami` (2.1.210) — tool patterns bind a command line's head, not execution. `command_guard` normalizes argv +
  enforces per-command schemas via a PreToolUse hook (deny beats allowedTools; e2e: `;`/`$()` denied, schema match ran).
  PR write is no longer standing — repo/permission-scoped GitHub App token minted only after approval-hash re-verification,
  then revoked + audited. App unregistered → `pr` execute fails closed.
- **D20 capability taxonomy + risk aggregation (local/CI, 2026-07-16):** old substring classifier scored `git add`/`checkout`/
  `pytest`/`terraform plan` as *no* capability. Now declared 5-class taxonomy, chain-summed risk vs `RISK_CEILING=10`
  (write-high/privileged exceed alone → L2/privileged blocked by arithmetic), score/ceiling/account/region pinned in the
  hashed plan, re-approval on read→write escalation/score/account/region change, unclassified tool = import error.
  `pr` risk 6, `tf-review` 1. Still allowlist-derived, pr-scoped.
- **D21 full-trajectory audit (local/CI, 2026-07-16):** audit events carry step_id(store-assigned)/parent_step_id/
  tool_name/capabilities/target_resource/result_hash; worker emits a tree (claim = root; write_credentials_issued
  descends from its approval step). Trajectory fields hash only when set → chains already in DynamoDB still verify.
  Sqlite↔DynamoDB moto-equivalent; web mirror updated. **Phase-level only** — per-tool-call sub-steps need stream-json.
- **All user-facing text English** (H0): agent Slack/chat and dashboard UI are English; dashboard agent seed rationales were translated (2026-07-15).
...
- **agent autonomous proposal loop (D9)** — extends the control plane to a shared human+agent producer.
  mcp_server (propose_job/list_pending — FastMCP server=slackops, permissions default-deny reuse), agent_monitor
  (Tier1 simulator rules + Tier2 real `claude -p --mcp-config`). Reuses the existing output gate (no new store state):
  proposal = PENDING/source=agent, L1 awaits human approval via await_approval. store adds JobSource.AGENT +
  Job.rationale. web/ has a human producer + agent badge/rationale. Runbook docs/runbooks/agent-mcp-demo.md.
