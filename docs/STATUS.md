...
- **D17 role/metadata/egress hardening (code-ready, not deployed):** bootstrap-only Instance Profile + separate 1-hour
  ...
  and use localhost Squid domain allowlist. New EC2 rehearsal must prove role identities, refresh, proxy allow/deny, and adapter read;
  prior D4 MCP proof is historical.
