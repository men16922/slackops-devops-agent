...
## 검증 Baseline
- `python3 -m pytest tests/ -q` → **183 passed, 1 skipped**(fastapi 미설치 로컬 한정 skip).
...
- stub 잔여: telemetry OTel 파이프라인(setup_telemetry) / commands(tf_review·pr) / worker.py(미작성).

## Active Focus
- H0 [auto] 트랙: worker.py 폴링 루프 → commands/{tf_review,pr}.
