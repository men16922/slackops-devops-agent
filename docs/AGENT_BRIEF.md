...
- **동작:** 명령 라우팅 + ping + job queue + permission gate + sanitizer + claude_runner(로컬 검증).
  AWS/Slack 실행분은 deploy/ 준비 완료.
- **검증:** `python3 -m pytest tests/ -q` → 76 passed, 1 skipped.
- **현재 초점:** Day 1–3 잔여(운영자 수동, deploy/README.md) + Day 4–5 잔여(allowlist + logs/diagnose).
